 ...
 * Starting the tests
 */
describe('Test when server booting', () => {
  beforeAll(async () => {
  ...
  it('When server booting switch processing job to failed ', async () => {
    const responseCreateJob = await createJob(client, {
      type: 'processingToFail',
      status: 'processing',
    })

    expect(responseCreateJob.status).toBe('processing')

    await new Promise((resolve) => server.close(() => resolve()))
    server = await getNewServer()

    const responseJobList = await request(server)
      .post('/graphql')
      .send(
        jobList({
          where: { type: 'processingToFail' },
        })
      )
