...
function getNewClient(uri, wsUri, apolloClientOptions) {
    ...
    if (typeof wsUri === 'undefined') {
        return new client_1.ApolloClient(__assign({ ssrMode: true, link: httpLink, cache: cache }, apolloClientOptions));
    }
    ...
    return new client_1.ApolloClient(__assign({ ssrMode: true, link: link, cache: cache }, apolloClientOptions));
}
